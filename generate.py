...
def nltk_map_synsets_to_id(wn_dist_path):
  SYN_TO_ID = {}
  ID_TO_SYN = [syn.name() for syn in wn.all_synsets()]
  SYNS_COUNT = 0

  syn_2_id_file = os.path.join(wn_dist_path, 'synset2id.tsv')
...
def nltk_map_relations_to_id(wn_dist_path):
  REL_TO_ID = {}
  ID_TO_REL = [
  ...
  rel_2_id_file = os.path.join(wn_dist_path, 'relation2id.tsv')
...
def nltk_map_edges_to_id(wn_dist_path, syn_to_id, rel_to_id):
  EDGES_COUNT = 0

  edge2id_all_file = os.path.join(wn_dist_path, 'edge2id_all.tsv')
  with open(edge2id_all_file, 'w+') as f_all:
    ...
    for rel_name in rel_to_id:
      ...
      edge2id_rel_file = os.path.join(wn_dist_path, 'edge2id_' + rel_name + '.tsv')
...
def main():
  ...
  SYN_TO_ID, _, SYNS_COUNT = nltk_map_synsets_to_id(wn_dist_path)
  REL_TO_ID, _, RELS_COUNT = nltk_map_relations_to_id(wn_dist_path)
  EDGES_COUNT = nltk_map_edges_to_id(wn_dist_path, SYN_TO_ID, REL_TO_ID)
